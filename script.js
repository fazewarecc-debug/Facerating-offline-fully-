...
async function loadModel(){
  ...
  model = await faceLandmarksDetection.load(
    ...
    {modelUrl:'models/mediapipe_facemesh/model.json'}
  );
...
analyzeBtn.addEventListener('click', async ()=>{
  ...
  if(!model) return alert("Модель ещё загружается.");
  ...
  img.onload = async ()=>{
    ...
    try{ await analyzeFace(img); }
    catch(err){ alert("Не удалось распознать лицо"); console.error(err); }
  };
